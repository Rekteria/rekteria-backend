...
router.get('/support', async (req, res) => {
  const getSupportList = await players.findAll({
    where: {
      group_id: {
        [Op.gt]: 3,
      },
  ...
  return res.jsonOK(getSupportList);
});

router.get('/character/:name', async (req, res) => {
  const { name } = req.params;
  const limit = 5;
  let getAllAchievement;
  ...
  if (searchCharacter.rows.length > 0) {
    getAllAchievement = await player_storage.findAndCountAll({
      where: {
    ...
    getCharacterList = await players.findAll({
      attributes: ['name'],
      where: {
...
router.post('/', checkJwt, validateCreateCharacter, async (req, res) => {
  ...
  const { name, vocation, sex, town_id } = body;
