...
router.delete('/:id/invite', checkJwt, async (req, res) => {
  try {
    ...
    const getRankLeader = await players.findAll({ where: { account_id } });
    const getPlayerAccounts = getRankLeader.map((p) => p);
    ...
    if (
      ...
      // const getInvites = await guild_invites.findAll({
      //   where: { guild_id: id, player_id: player_id },
      // });

      console.log(checkVice);
    } else {
